-- truncate partititon by batch table
{% macro truncate_by_part(src_tab, tgt_tab, trunc_clause) %}
...
#}
    {% if execute and flags.WHICH in ["run", "test", "seed", "snapshot", "run-operation"] and load_relation(tgt_tab) is not none %}
        ...
        {{ add_missing_partitions(src_clause, tgt_tab, prt_type, prt_col_type) }}
        {{ truncate_partition_raw(src_clause, tgt_tab, prt_type, prt_col_type) }}
    {% endif %}
{% endmacro %}