{% macro test_dq0__v_pos_txn_ucpn_ecom__unique_rows(model) %}
{% set model = '' %}
    ...
    check_todays_rows AS (
        ...
        LEFT JOIN count_rows cr
            ON d.txn_save_dt = cr.txn_save_dt
    ),
    mediana_14_days AS (
        SELECT
            median(rows_per_day) AS mediana_14_days
        FROM count_rows
    ...
    SELECT
        COALESCE(
            STRING_AGG(
                concat(txn_save_dt, ' : ', rows_per_day),
                ', '
            ),
            '0'
        ) AS anomaly_result,
        ...
        (
            SELECT todays_rows_is_empty
            FROM check_todays_rows
        ) AS todays_rows_is_empty,
