{% macro test_dq0__cheque__instance(model) %}
{% set model = '' %}
    ...
    with
        stat as (
            select
                ...
                min(scc.datetime) as min_date,
                max(scc.datetime) as max_date
            from (
                select
                h.instance_id,
                s.dt_load,
                s.datetime,
                row_number() over (partition by h.instance_id, h.cheque_pk order by s.version_id desc) as rn
                from {{ ref('sat_cheque') }} s
                join {{ ref('hub_cheque') }} h on h.cheque_pk = s.cheque_pk
