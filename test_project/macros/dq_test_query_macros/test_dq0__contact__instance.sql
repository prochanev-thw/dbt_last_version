{% macro test_dq0__contact__instance(model) %}
{% set model = '' %}
    ...
    with
        stat as (
            ...
            from (
                ...
                sc.dt_load,
                sc.modified_on,
                row_number() over (partition by hc.instance_id, hc.contact_id order by sc.version_id desc) as rn
                from {{ ref('sat_contact_crmdata') }} sc
                JOIN {{ ref('hub_contact') }} hc ON hc.contact_pk = sc.contact_pk
                where sc.dt_load >= '{{ var("dm_date") }}'::date - interval '8 day'
                and sc.dt_load < '{{ var("dm_date") }}'::date) scc
